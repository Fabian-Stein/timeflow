...
press() {
if (cState == SI.play) pause();
else if (cState == SI.pause) play();
else {
cState = SI.play;
t.start();
}
...
return NumberPickerDialog.integer(initialIntegerValue: (dur+1.0) ~/ 60000, maxValue: 20, minValue: 1, title: Text('Minutes'));
}).then((num v) {
...
w.add(SizedBox(height: 10));
w.add(fab(Colors.yellow[900], () {rebuild = true;}, Icons.loop));
w.add(SizedBox(height: 20));
}
...
FloatingActionButton fab(Color c, VoidCallback f, IconData ic) => FloatingActionButton(backgroundColor: c, onPressed: f, child: Icon(ic));
...
var x = (rng.nextDouble()-0.5)*(wR-0.1), y = (rng.nextDouble()-0.5)*(hR-0.1);
dP[0] = Point(x, y);
for (i = 1; i < 3; i++)
...
for (i = 0; i < 3; i++)
res[i] = Point(sP[i].x*p+dP[i].x*(1-p), sP[i].y*p+dP[i].y*(1-p));
